...
class HeadController:
    ...
    def look_up(self, deg: float = 15.0, duration_s: float = 0.35) -> None:
        """Tilt head up (-15° = up)."""
        self.look_at(self.current_pan, -abs(deg), duration_s=duration_s)

    def look_down(self, deg: float = 15.0, duration_s: float = 0.35) -> None:
    ...
    def nod(self, count: int = 2, amplitude_deg: float = 12.0) -> None:
        ...
        for _ in range(count):
            self.look_at(self.current_pan, amp, duration_s=0.18)        # Down (+)
            self.look_at(self.current_pan, -amp * 0.7, duration_s=0.18) # Up (-)
    ...
    def shake(self, count: int = 2, amplitude_deg: float = 20.0) -> None:
        ...
        for _ in range(count):
            self.look_at(amp, self.current_tilt, duration_s=0.18)   # Left (+)
            self.look_at(-amp, self.current_tilt, duration_s=0.18)  # Right (-)
        self.look_center(duration_s=0.2)
    ...
    def track_bounding_box(
        ...
        # Adjust pan: If face is to the right in the frame (err_x > 0), turn right (negative).
        # If face is to the left in the frame (err_x < 0), turn left (positive).
        delta_pan = -err_x * 40.0 * gain

        # Adjust tilt: If face is higher up in frame (err_y < 0), tilt up (negative).
        # If face is lower down in frame (err_y > 0), tilt down (positive).
        delta_tilt = err_y * 15.0 * gain
